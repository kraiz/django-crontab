...
from django_cronjobs.app_settings import CRONTAB_EXECUTABLE, CRONJOBS, \
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if len(args) > 0:
            if args[0] == 'add':
                ...
                self.__remove_cronjobs(**options)
                self.__add_cronjobs(**options)
    ...
    def __add_cronjobs(self, **options):
        ...
        for cronjob in CRONJOBS:
            self.crontab_lines.append(CRONTAB_LINE_PATTERN % {
                ...
                'command': '%(prefix)s %(exec)s %(manage)s cronjobs run %(jobname)s %(suffix)s' % {
    ...
    def __remove_cronjobs(self, **options):
